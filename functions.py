...
def gamma_builder_L1(y: float, tau_for_new: int) -> List[QuadPiece]:
    ...
    return [(-INF, y, 0.0, -1.0, float(y), tau_for_new), (y, INF, 0.0, 1.0, float(-y), tau_for_new)]
...
def compute_penalty_beta(y, loss):
    ...
    if loss == "l2":
        ...
        E_phi2, _ = integrate.quad(
            lambda z: (biweight_phi(z, K_std) ** 2) * norm.pdf(z), -np.inf, np.inf
        )
        ...
        K_std = 1.345
        E_phi2, _ = integrate.quad(
            lambda z: (huber_phi(z, K_std) ** 2) * norm.pdf(z), -np.inf, np.inf
        )
        return 2 * sigma**2 * np.log(n) * E_phi2
...
def cross_validate_rfpop(
    ...
    if verbose:
        ...
        tqdm.write(f"Cross-validation ({criterion}) pour {loss} loss")
        tqdm.write(f"Exploration de {len(beta_range)} × {len(K_range)} = {total} combinaisons...")
...
def plot_sensitivity_tobeta(
    ...
    for i, loss in tqdm(enumerate(["huber", "biweight", "l2"])):
        ...
        for scaling in list_scaling:
            if loss == "huber":
                cp_tau, Qt_vals, _qstar_final = rfpop_algorithm1_main(
                    y, lambda y_t, t: gamma_builder_huber(y_t, K, t), scaling
                )
            elif loss == "biweight":
                cp_tau, Qt_vals, _qstar_final = rfpop_algorithm1_main(
                    y, lambda y_t, t: gamma_builder_biweight(y_t, K, t), scaling
                )
            elif loss == "l2":
                cp_tau, Qt_vals, _qstar_final = rfpop_algorithm1_main(
                    y, lambda y_t, t: gamma_builder_L2(y_t, t), scaling
                )
...
def plot_segments(df, name, scaling_huber, scaling_biweight, scaling_l2):
    ...
    for i, loss in tqdm(enumerate(["huber", "biweight", "l2"])):
        ...
        if loss == "huber":
            cp_tau, Qt_vals, _qstar_final = rfpop_algorithm1_main(
                y, lambda y_t, t: gamma_builder_huber(y_t, K, t), beta * scaling_huber
            )
        elif loss == "biweight":
            cp_tau, Qt_vals, _qstar_final = rfpop_algorithm1_main(
                y, lambda y_t, t: gamma_builder_biweight(y_t, K, t), beta * scaling_biweight
            )
        elif loss == "l2":
            cp_tau, Qt_vals, _qstar_final = rfpop_algorithm1_main(
                y, lambda y_t, t: gamma_builder_L2(y_t, t), beta * scaling_l2
            )
...
def online_most_recent_changepoint(
    ...
    if loss == "huber":
        ...
        def gamma_builder(y_t, t, K=K):
            return gamma_builder_huber(y_t, K, t)

    elif loss == "biweight":

        def gamma_builder(y_t, t, K=K):
            return gamma_builder_biweight(y_t, K, t)
...
def plot_most_recent_changepoint(
    online_result, y=None, dates=None, true_changepoints=None, title=""
):
    """
    ...
    ax.set_title(
        f'{title}\nOnline Analysis - {online_result["params"]["loss"].upper()} loss', fontsize=14
    )
...
__all__ = [
    ...
    "generate_scenarios",
    "cross_validate_rfpop",
    "plot_sensitivity_tobeta",
    "plot_segments",
    "online_most_recent_changepoint",
    "plot_most_recent_changepoint",
]

_legacy_api_references = (
    generate_scenarios,
    cross_validate_rfpop,
    plot_sensitivity_tobeta,
    online_most_recent_changepoint,
